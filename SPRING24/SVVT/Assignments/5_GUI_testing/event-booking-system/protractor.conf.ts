/*
 * -----------------------------------------------------------------------
 * File: protractor.conf.js
 * Creation Time: Apr 16th 2024, 8:25 pm
 * Author: Saurabh Zinjad
 * Developer Email: saurabhzinjad@gmail.com
 * Copyright (c) 2023-2024 Saurabh Zinjad. All rights reserved | https://github.com/Ztrimus
 * -----------------------------------------------------------------------
 */
let SpecReporter = require('jasmine-spec-reporter').SpecReporter;
const failFast = require('protractor-fail-fast');

exports.config = {
  directConnect: true, // lets Protractor connect directly to the browser without using a Selenium server
  framework: 'jasmine', // using the Jasmine test framework
  specs: [
    'src\\app\\event-listing\\event-listing.component.spec.ts',
    'src\\app\\event-registration\\event-registration.component.spec.ts',
    'src\\app\\confirmation\\confirmation.component.spec.ts',
  ], // specifies where your test files are located
  chromeDriver: 'C:\\Program Files\\Google\\Chrome\\chromedriver.exe',
  capabilities: {
    browserName: 'chrome', // specifies which browser to use for testing
  },
  jasmineNodeOpts: {
    defaultTimeoutInterval: 30000, // how long to wait before a test fails
  },
};
